...
$sid = (isset($_POST['student']))?mysqli_real_escape_string($conn,$_POST['student']):'';
 ...
 $sql = "select s.id,s.sname,s.balance,s.fees,s.contact,b.grade,s.transid from student as s,grade as b where b.id=s.grade and  s.delete_status='0' and s.id='".$sid."'";
$q = $conn->query($sql);
if($q->num_rows>0)
{
...
if($fq->num_rows>0)
{
 ...
 $sql = "select s.id,s.sname,s.balance,s.fees,s.contact,b.grade,s.transid from student as s,grade as b where b.id=s.grade  and s.id='".$sid."'";
$sq = $conn->query($sql);
$sr = $sq->fetch_assoc();
